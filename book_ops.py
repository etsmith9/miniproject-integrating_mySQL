...
# Check if connection was successful, and create cursor
if conn is not None:
    cursor = conn.cursor()  # Create the cursor only if the connection is successful
else:
...
class Book:
    ...
    @staticmethod
    def add_book(cursor, title, author, genre, ISBN):
        if cursor is None:
            print("Error: Cursor is not initialized. Cannot add book.")
            return

        while True:  # Validate ISBN format
            ISBN = input("Please enter the ISBN of the book (xxx-x): ")
    ...
    @staticmethod
    def borrow_book(cursor, ISBN):
        if cursor is None:
            print("Error: Cursor is not initialized. Cannot borrow book.")
            return
    ...
    @staticmethod
    def return_book(cursor, ISBN):
        if cursor is None:
            print("Error: Cursor is not initialized. Cannot return book.")
            return
    ...
    @staticmethod
    def search_for_book(cursor, ISBN):
        if cursor is None:
            print("Error: Cursor is not initialized. Cannot search for book.")
            return
    ...
    @staticmethod
    def display_book(cursor):
        if cursor is None:
            print("Error: Cursor is not initialized. Cannot display books.")
            return
...
while True:
    ...
    try:
        ...
        if book_menu_action == 1:
            Book.add_book(cursor, 'title', 'author', 'genre', 'ISBN')
        elif book_menu_action == 2:
            Book.borrow_book(cursor, 'ISBN')
        elif book_menu_action == 3:
            Book.return_book(cursor, 'ISBN')
        elif book_menu_action == 4:
            Book.search_for_book(cursor, 'ISBN')
        elif book_menu_action == 5:
            Book.display_book(cursor)
...
if cursor:  # Check if cursor is defined before closing
    cursor.close()
if conn:  # Check if connection is defined before closing
    conn.close()
